...
class Level(tool.State):
    def __init__(self):
        ...
        # Никаких сокетов здесь не создаём!
        # Порты будут открываться и закрываться внутри get_ai_observation
    ...
    def update(self, surface, current_time, mouse_pos, mouse_click):
        ...
        if self.state == c.PLAY and not self.done:
            self.get_ai_observation()
        if self.state == c.PLAY:
            for i in range(5):
                for zombie in self.zombie_groups[i]:
                    if zombie.rect.x <= 80:
                        print("\n[ИИ ДВИЖОК] Зомби прорвали линию! Отправляем сигнал RESET...")
                        try:
                            import socket, json
                            # Рассылаем RESET всем агентам на все порты
                            for port in [5005, 5007, 5008, 5009]:
                                try:
                                    sync_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                                    sync_sock.sendto(json.dumps({"status": "RESET"}).encode('utf-8'), ("127.0.0.1", port))
                                    sync_sock.close()
                                except:
                                    pass
                        except:
        ...
        if self.state == c.CHOOSE:
            ...
            if hasattr(self, 'panel') and self.panel is not None:
                try:
                    import source.component.menubar as menubar_mod
                    all_cards = getattr(menubar_mod, 'all_card_list', [])
                    card_list_to_play = []
                    for card in all_cards:
                        card_list_to_play.append(card)
                        if len(card_list_to_play) >= 8:
                            break
                    self.initPlay(card_list_to_play)
                    print("[ИИ АВТОСТАРТ] Карты успешно выданы! Бой начался.")
    ...
    def get_ai_observation(self):
        ...
        try:
            for sun in self.sun_group:
                if hasattr(sun, 'state') and sun.state != 'die':
                    self.menubar.increaseSunValue(sun.sun_value)
                    sun.kill()
        except:
            pass

        grid = np.zeros((5, 9), dtype=np.float32)
        for map_y in range(5):
            for map_x in range(9):
                cell_value = self.map.map[map_y][map_x]
                if cell_value != 0:
                    grid[map_y][map_x] = 1.0

        zombie_positions = np.full(5, 800.0, dtype=np.float32)
        for i in range(5):
            for zombie in self.zombie_groups[i]:
                if zombie.rect.x < zombie_positions[i]:
                    zombie_positions[i] = float(zombie.rect.x)

        data_to_send = {
            "grid": grid.flatten().tolist(),
            "zombies": zombie_positions.tolist(),
            "sun": float(self.menubar.sun_value)
        }

        # Слушаем порты динамически (без предварительного bind)
        ports = [5006, 5007, 5008, 5009]
        for port in ports:
            try:
                temp_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                temp_sock.settimeout(0.02)  # Ждем 20 мс на ответ от нейросети
                temp_sock.bind(("127.0.0.1", port))

                try:
                    data, addr = temp_sock.recvfrom(1024)
                    response = json.loads(data.decode('utf-8'))
                    action = response.get("action", -1)

                    # Обработка действия
                    if 0 <= action < 45:
                        map_y = action // 9
                        map_x = action % 9
                        import source.component.plant as plant
                        x, y = self.map.getMapGridPos(map_x, map_y)
                        if self.menubar.sun_value >= 100 and self.map.map[map_y][map_x] == 0:
                            new_plant = plant.PeaShooter(x, y, self.bullet_groups[map_y])
                            self.menubar.decreaseSunValue(100)
                            self.plant_groups[map_y].add(new_plant)
                            self.map.map[map_y][map_x] = 1
                            print(f"--> [БОЙ!] PeaShooter на [{map_y}, {map_x}] от порта {port}")
                        else:
                            print(f"--> [ОТКАЗ] Нет денег или клетка занята [{map_y}, {map_x}]")
                        ...
                        import source.component.plant as plant
                        x, y = self.map.getMapGridPos(map_x, map_y)
                        if self.menubar.sun_value >= 50 and self.map.map[map_y][map_x] == 0:
                            new_plant = plant.SunFlower(x, y, self.sun_group)
                            self.menubar.decreaseSunValue(50)
                            self.plant_groups[map_y].add(new_plant)
                            self.map.map[map_y][map_x] = 1
                            print(f"--> [ЭКОНОМИКА] SunFlower на [{map_y}, {map_x}] от порта {port}")
                        else:
                            print(f"--> [ОТКАЗ] Нет денег или клетка занята [{map_y}, {map_x}]")

                    # Отправляем ответ обратно на тот же порт, с которого пришла команда
                    temp_sock.sendto(json.dumps(data_to_send).encode('utf-8'), ("127.0.0.1", port))

                except socket.timeout:
                    pass
                except Exception as e:
                    print(f"[ОШИБКА] get_ai_observation: {e}")
                finally:
                    temp_sock.close()
            except:
                pass