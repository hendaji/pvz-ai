...
class Level(tool.State):
	def __init__(self):
		...
		self.ai_sock.bind(("127.0.0.1", 5006))
		self.ai_sock.setblocking(False)
				...
				for zombie in self.zombie_groups[i]:
					if zombie.rect.x <= 80:
						...
						try:
							...
							sync_sock.sendto(json.dumps({"status": "RESET"}).encode('utf-8'), ("127.0.0.1", 5005))
							sync_sock.close()
						except:
							pass
						self.next = c.LEVEL
	...
	def get_ai_observation(self):
		print("[ДЕБАГ] get_ai_observation() вызвана!")
		import numpy as np
		import json
		import socket
		...
		try:
			...
			send_sock.sendto(message, ("127.0.0.1", 5005))
			send_sock.close()

			try:
				data, addr = self.ai_sock.recvfrom(1024)
				...
				if 0 <= action < 45:
					...
					if self.menubar.sun_value >= 100 and self.map.map[map_y][map_x] == 0:
						...
						print(f"--> [БОЙ!] PeaShooter на [{map_y}, {map_x}]")
					else:
					...
					if self.menubar.sun_value >= 50 and self.map.map[map_y][map_x] == 0:
						...
						self.plant_groups[map_y].add(new_plant)
						self.map.map[map_y][map_x] = 1
						print(f"--> [ЭКОНОМИКА] SunFlower на [{map_y}, {map_x}]")
					else:
						print(f"--> [ОТКАЗ] Нет денег или клетка занята [{map_y}, {map_x}]")
